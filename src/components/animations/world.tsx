...
import * as THREE from 'three';
  ...
  function render(globe, renderer, scene, camera, width, height, gl, pixels) {
    requestAnimationFrame(render(globe, renderer, scene, camera));
    globe.rotation.y -= 0.01;
    renderer.render(scene, camera);
    gl.readPixels(0, 0, width, height, gl.RGBA, gl.UNSIGNED_BYTE, pixels)
    var text = grayscale10(pixels).map(asciify).join("");
...
export default component$(() => {
    ...
    // go through earthlines are print them with whitespace:
     return(
            <div>
                {earth_lines.map((line) => {
                    return <div className="font-mono whitespace-pre tracking-widest">{line}</div>
                })}
                </div>
     )
})
